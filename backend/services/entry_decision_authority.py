...
def build_day_entry_provenance(
    *,
    ...
    return {
        ...
        "decision_id": str(decision_id or dd.get("decision_id") or opp),
        "inference_log_id": dd.get("inference_log_id") or dd.get("ai_inference_log_id"),
        "signal_timestamp": dd.get("signal_timestamp") or dd.get("signal_content_timestamp") or dd.get("ctx_ts_utc"),
        "decision_timestamp": dd.get("decision_timestamp") or dd.get("prediction_timestamp") or bar_timestamp,
        "p_buy": _num(dd.get("prob_buy") if dd.get("prob_buy") not in (None, "") else dd.get("p_buy")),
        "p_sell": _num(dd.get("prob_sell") if dd.get("prob_sell") not in (None, "") else dd.get("p_sell")),
        "p_hold": _num(dd.get("prob_hold") if dd.get("prob_hold") not in (None, "") else dd.get("p_hold")),
        "ml_score": _num(dd.get("ml_score") if dd.get("ml_score") not in (None, "") else dd.get("model_score")),
        "predicted_gross_ev": _num(dd.get("predicted_gross_ev") if dd.get("predicted_gross_ev") not in (None, "") else dd.get("model_gross_ev")),
        "predicted_cost": _num(dd.get("predicted_cost") if dd.get("predicted_cost") not in (None, "") else dd.get("estimated_fees_pct")),
        "predicted_net_ev": buy_ev,
        "final_selection_score": _num(dd.get("final_selection_score") if dd.get("final_selection_score") not in (None, "") else rank_score),
        "universe_rank": dd.get("universe_rank") if dd.get("universe_rank") not in (None, "") else dd.get("selected_rank"),
        "features_json_hash": dd.get("features_json_hash") or dd.get("feature_fingerprint") or feature_fingerprint,
        "ai_inference_log_id": dd.get("ai_inference_log_id") or dd.get("inference_log_id"),
        "prior_4h_low": _num(dd.get("prior_4h_low")),
        "distance_to_prior_4h_low_bps": _num(dd.get("distance_to_prior_4h_low_bps")),
        "forming_4h_bar_age_min": _num(dd.get("forming_4h_bar_age_min")),
        "time_to_4h_boundary_min": _num(dd.get("time_to_4h_boundary_min")),
        "ema_alignment": _num(dd.get("ema_alignment") if dd.get("ema_alignment") not in (None, "") else dd.get("signal_ema_alignment")),
        "thesis_invalid_components": dd.get("thesis_invalid_components"),
        "spread": _num(dd.get("spread") if dd.get("spread") not in (None, "") else dd.get("spread_pct")),
        "slippage_estimate": _num(dd.get("slippage_estimate") if dd.get("slippage_estimate") not in (None, "") else dd.get("estimated_slippage_pct")),
        "binance_order_id": dd.get("binance_order_id") or dd.get("exchange_order_id"),
        "binance_fill_id": dd.get("binance_fill_id") or dd.get("exchange_fill_id"),
        "attribution_join_status": dd.get("attribution_join_status") or "live",
    }
