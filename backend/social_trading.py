#!/usr/bin/env python3
"""
Live trade pattern analyzer for Binance.US 1m klines.
- Python 3.12, Windows 11 Pro compatible
- No static data — connects directly to Binance.US websocket stream
- Symbols default: BTCUSDT, ETHUSDT, ADAUSDT, SOLUSDT, DOGEUSDT, XRPUSDT, BCHUSDT, LTCUSDT, AVAXUSDT, LINKUSDT
- Computes: trade tag (breakout/breakdown/uptrend/downtrend/consolidation), trend, strength, volatility,
  RSI-14, moving average slope, and strategy confidence (optionally boosted by 'mystic' signals from Redis)
- Publishes JSON lines to stdout in real time and (if Redis available) to channel 'mystic:stream:trade_insights'

Requirements (install once in your venv):
    pip install websockets redis

Note:
- Only Binance.US is used (wss://stream.binance.us:9443)
- Uses closed 1-minute klines (outputs once per minute per symbol)
- If Redis is running on localhost:6379, mystic signals can be read from keys:
    mystic:signal:tesla_369, mystic:signal:faerie_star, mystic:signal:mystic_alignment_score
  (All are optional; if absent, zero-boost.)
"""
...
def tag_trade(price: float, recent_prices: Sequence[float], lookback: int = 50) -> str:
    ...
    if not window:
        return "unknown"
    prev_window = window[:-1] if len(window) > 1 else window
    prev_last = prev_window[-1] if prev_window else window[-1]
    hi = max(prev_window) if prev_window else prev_last
    lo = min(prev_window) if prev_window else prev_last
    if price > hi:
        return "breakout"
    if price < lo:
        return "breakdown"
...
def analyze_trade_pattern(prices: Sequence[float], _volumes: Sequence[float] | None = None) -> dict[str, Any]:
    ...
    if ema_fast is not None and ema_slow is not None:
        rel_diff = (abs(ema_fast - ema_slow) / ema_slow) if ema_slow else 0.0
        ma_slope = "flat" if rel_diff < 0.0005 else "rising" if ema_fast > ema_slow else "falling"
    else:
        ma_slope = "flat"
...
class MysticSignalSource:
    ...
    def connect(self) -> None:
        if redis is None:
            logging.info("redis package not installed; mystic signals disabled")
            return
        try:
            ...
            self._client.ping()
            logging.info("Connected to Redis via shared pool")
        except (ValueError, TypeError, AttributeError, KeyError, IndexError, RuntimeError) as e:
            logging.warning("Redis unavailable (%s); proceeding without mystic signals", e)
            self._client = None
    ...
    def get_latest(self) -> dict[str, float]:
        ...
        try:
            ...
            logging.debug("Redis read error: %s", e)
            return {}
        else:
            return out
    ...
    def publish(self, channel: str, message: str) -> None:
        ...
        try:
            ...
            logging.debug("Redis publish error: %s", e)
...
class LiveKlineWatcher:
    ...
    async def run(self) -> None:
        ...
        while not self._stop:
            try:
                logging.info("Connecting: %s", self.url)
                async with websockets.connect(self.url, ping_interval=20, ping_timeout=20) as ws:
                    logging.info("Connected to Binance.US stream (%s)", self.streams)
                    backoff = 1.0
                    async for raw in ws:
                        try:
                            msg = json.loads(raw)
                            ...
                            logging.debug("Message handling error: %s", e)
            except asyncio.CancelledError:
                raise
            except (ValueError, TypeError, AttributeError, KeyError, IndexError, RuntimeError) as e:
                logging.warning("Websocket error: %s — reconnecting in %.1fs", e, backoff)
                await asyncio.sleep(backoff)
                backoff = min(max_backoff, backoff * 1.7)  # Remove random component
    ...
    def _handle_message(self, msg: dict[str, Any]) -> None:
        ...
        # Print to stdout (one JSON line per closed kline) and also publish to Redis if available
...
async def _amain() -> None:
    ...
    task = await task_manager.create_task(watcher.run(), name="social_trading:run")
    try:
        await task
