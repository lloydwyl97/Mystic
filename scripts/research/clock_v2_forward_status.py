...
filled_rows = conn.execute("SELECT COUNT(*) FROM day_decision_candidate_records WHERE action_contract_version IS NOT NULL").fetchone()[0]
print(f"  rows total={tot:,}  with corrected contract={filled_rows:,}")
for r in conn.execute(
    "SELECT action_available, action_unavailable_reason, COUNT(*) n FROM day_decision_candidate_records WHERE action_contract_version IS NOT NULL GROUP BY 1,2 ORDER BY n DESC LIMIT 8"
):
...
bad = conn.execute("SELECT COUNT(*) FROM day_decision_candidate_records WHERE production_selected = 1 AND action_available = 0").fetchone()[0]
sel = conn.execute("SELECT COUNT(*) FROM day_decision_candidate_records WHERE production_selected = 1").fetchone()[0]
...
if table_exists("day_path_clock_v2_candidate_artifact"):
    for r in conn.execute("SELECT clock_v2_partition, COUNT(DISTINCT decision_group_id) g, COUNT(*) rows FROM day_path_clock_v2_candidate_artifact GROUP BY 1"):
        print(f"  partition={r[0]!s:<22} groups={r[1]:<5} rows={r[2]}")

print("\n=== planned v5 in experiment registry ===")
if table_exists("day_experiment_registry"):
    for r in conn.execute("SELECT experiment_id, timestamp, result, promoted FROM day_experiment_registry ORDER BY timestamp"):
...
if table_exists("day_clock_v2_outcome_labels"):
    ...
    reasons = Counter(r[0] for r in conn.execute("SELECT label_invalid_reason FROM day_clock_v2_outcome_labels WHERE label_valid=0"))
...
if table_exists("day_forward_lock_registry"):
    for r in conn.execute("SELECT experiment_id, inspected FROM day_forward_lock_registry ORDER BY experiment_id"):
