...
def main() -> int:
    ...
    span_days_actual = int((bars_1h[SYMBOLS[0]][-1]["ts"] - bars_1h[SYMBOLS[0]][0]["ts"]) / 86400)
    ...
    btrades = [t for t in brep.get("trades_detail", []) if t.get("exit_reason") != "REPLAY_MARK_TO_MARKET"]
    ...
    bounded_probe = {
        ...
        "per_regime": {r: _seg_stats([t for t in btrades if t["regime"] == r]) for r in sorted({t["regime"] for t in btrades})},
        "note": (
    ...
    print(
        f"    bounded: trades={len(btrades)} net=${bounded_probe['net_pnl_usd']} monthly=${bounded_probe['monthly_pnl_usd']} win%={bounded_probe['win_rate_pct']}",
        flush=True,
    )
    ...
    for mult in NOTIONAL_MULTS:
        ...
        trades = [t for t in rep.get("trades_detail", []) if t.get("exit_reason") != "REPLAY_MARK_TO_MARKET"]
        ...
        for bid, btr in sorted(per_bucket.items()):
            ...
            accept = st["trades"] >= MIN_BUCKET_TRADES and st["net_pnl_usd"] > 0 and st["expectancy_per_trade_usd"] > 0 and mfrac >= ACCEPT_MONTH_FRAC and st["longest_hold_hours"] <= 72.0
            if accept:
                accepted.append(bid)
            bucket_rows.append(
                {
                    "bucket": bid,
                    **st,
                    "months_traded": mtraded,
                    "months_positive": mpos,
        ...
        profile_results.append(
            {
                ...
                "accepted_allweather_set": {
                    ...
                    "per_regime": {r: _seg_stats([t for t in acc_trades if t["regime"] == r]) for r in sorted({t["regime"] for t in acc_trades})},
                },
    ...
    gate_metrics = {
        ...
        "expectancy_per_trade_usd": (round(acc["net_pnl_usd"] / acc["total_trades"], 2) if acc["total_trades"] else 0.0),
        "longest_hold_hours": max((s.get("longest_hold_hours", 0) for s in acc["per_regime"].values()), default=0.0),
    ...
    report = {
        ...
        "exec_interval": EXEC_INTERVAL,
        "method_note": ("Unrestrained explore across full multi-year span; buckets accepted on own-regime profitability + month-consistency, not recent-window survival."),
        ...
        "verdict": {
            ...
            "next_step_if_target_met": ("Run full walk-forward + stress battery on accepted set before any live promotion."),
        },
    }
