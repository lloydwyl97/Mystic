...
class VerifyReport:
    ...
    cycle_stats: dict[str, int] = field(
...
def verify_cleanup(report: VerifyReport) -> None:
    ...
    eth_pos = conn.execute("SELECT COUNT(*) c FROM portfolio_engine_positions WHERE symbol LIKE '%ETH%'").fetchone()["c"]
    conn.execute("SELECT COUNT(*) c FROM portfolio_engine_positions").fetchone()["c"]
    ...
    repair_rows = conn.execute("SELECT COUNT(*) c FROM portfolio_engine_positions WHERE COALESCE(repair_add_count,0) > 0").fetchone()["c"]
    ...
    report.ok("eth_cleaned", eth_pos == 0 and eth_sell and eth_sell["exit_reason"] == "LEGACY_INVENTORY_CLEANUP_EXIT", f"eth_positions={eth_pos} last_exit={dict(eth_sell) if eth_sell else None}")
...
def verify_buy_scenarios(report: VerifyReport) -> None:
    ...
    scenarios = [
        ...
        (
            ...
            False,
            "REGIME_ROUTE_CHOP_NO_DAY",
        ),
        (
            "bear_pullback_blocked",
            DAY_REGIME_BEAR,
            SETUP_HTF_TREND_PULLBACK,
            {
                "mtf_json": _htf_bull_mtf(),
        ...
        (
            ...
            False,
            "REGIME_ROUTE_XRP_CHURN_CONFIRMATION",
        ),
    ]
    ...
    for name, regime, setup, dd, expect_allowed, expect_reason in scenarios:
        ...
        if not passed:
            all_buy_pass = False
            report.assertions_failed.append(f"buy_scenario_{name}: allowed={allowed} block={block} expected_allowed={expect_allowed}")
        buy_results.append(
            {
                "scenario": name,
                "regime": regime,
    ...
    try:
        ...
        for sym in TOP4:
            ...
            dd = apply_trade_thesis_to_candidate_fields(
                merged,
                symbol=sym,
                current_price=float(sig.get("current_price") or ctx.get("close") or 100),
                atr=1.0,
                strategy_id="day",
            )
            ...
            route = evaluate_day_entry_route(
                ...
                decision_data=dd,
                context_payload=ctx,
                current_price=float(sig.get("current_price") or 100),
            ...
            sym_rows.append(
                {
                    ...
                    "net_ev_after_fees": net_ev,
                    "historical_expectancy_pct": hist_exp,
                    "route_allowed": route.get("allowed"),
                    "block_reason": route.get("block_reason"),
                    "signal_side": sig.get("action") or sig.get("side"),
                }
            )
...
async def verify_engine_gates(report: VerifyReport) -> None:
    ...
    engine.open_positions["BTC/USDT"] = OpenPosition(
        ...
        trade_id="test",
        stop_price=60000,
        take_profit_1_price=66000,
        take_profit_2_price=68000,
    )
    ...
    engine.open_positions["SOL/USDT"] = OpenPosition(
        ...
        trade_id="test2",
        stop_price=140,
        take_profit_1_price=160,
        take_profit_2_price=165,
    )
...
def verify_ai(report: VerifyReport) -> None:
    ...
    try:
        from backend.services.ai_learning_ingestion import label_pending_snapshots, learning_health_summary
...
def verify_dashboard_db(report: VerifyReport) -> None:
    conn = sqlite3.connect(DB_PATH)
    ledger = conn.execute("SELECT cash_balance, total_equity, positions_value FROM portfolio_engine_ledger WHERE id=1").fetchone()
    ...
    try:
        ...
        match = abs(api_cash - float(ledger[0])) < 0.02 and abs(api_eq - float(ledger[1])) < 0.02 and api_open == open_db
